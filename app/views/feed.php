...
<body>
    <div class="post-container">
        ...
        $stmt = $pdo->query('
            SELECT 
                postnormal.id AS postnormal_id, 
                posts.id AS post_id, 
                users.username, 
                users.profile_pic, 
                postnormal.conteudo, 
                postnormal.image_path, 
                postnormal.file_path, 
                postnormal.youtube_link, 
                posts.created_at, 
                posts.user_id
            FROM postnormal
            JOIN posts ON postnormal.post_id = posts.id
        ...
        while ($row = $stmt->fetch()) {
            ...
            if ($row['profile_pic']) {
                ...
                echo "<img src='../../assets/profile_pics/perfil_sem_foto.png' class='mr-3 rounded-circle' alt='Default Profile Picture' style='width: 50px; height: 50px;'>";
            }
            ...
            if ($row['user_id'] == $user_id) {
                ...
                echo "    <div class='ellipsis' id='ellipsis-btn-" . $row['post_id'] . "'>";
                echo "        &#x2022;&#x2022;&#x2022;";
                echo "    </div>";

                echo "    <div class='dropdown' id='dropdown-menu-" . $row['post_id'] . "'>";
                echo "        <ul>";
                echo "            <li>";
                echo "                <form action='../controllers/edit_post.php' method='GET' style='display:inline;'>";
                echo "                    <input type='hidden' name='post_id' value='" . $row['post_id'] . "'>";
                echo "                    <input type='hidden' name='from' value='feed.php'>";
            ...
            if ($row['image_path']) {
                $imagePath = "../../" . ltrim($row['image_path'], "/");
                echo "<img src='" . htmlspecialchars($imagePath) . "' class='img-fluid rounded mb-3' alt='Post Image' style='width: 300px; height: auto;'>";
            }
            ...
            ');
            $comments_stmt->execute([$row['post_id']]);
            ...
            while ($comment = $comments_stmt->fetch()) {
                ...
                if ($comment['user_id'] == $user_id) {
                    ...
                    echo "            <li>";
                    echo "                <form action='../controllers/edit_comment.php' method='GET' style='display:inline;'>";
                    ...
                    echo "            <li>";
                    echo "                <form action='../controllers/delete_comment.php' method='POST' style='display:inline;'>";
                    echo "                    <input type='hidden' name='comment_id' value='" . $comment['id'] . "'>";
                    echo "                    <input type='hidden' name='from' value='feed.php'>";
                    echo "                    <button type='submit' class='btn btn-danger btn-sm'>Excluir</button>";
                    echo "                </form>";
            ...
            echo "<form action='../controllers/add_comment.php' method='POST'>";
            echo "<input type='hidden' name='post_id' value='" . $row['post_id'] . "'>";
            ...
            $stmt_likes->execute(['user_id' => $user_id, 'post_id' => $row["post_id"]]);
            ...
            $stmt_dislikes->execute(['user_id' => $user_id, 'post_id' => $row["post_id"]]);
        ...
        <script>
            // Menu 3 pontinhos dos POSTS
            document.querySelectorAll('[id^="ellipsis-btn-"]').forEach(ellipsisBtn => {
                const postId = ellipsisBtn.id.split('-')[2]; 
            ...
            // Menu 3 pontinhos dos COMENTÁRIOS
